...
SWORE = temp.SWORE(:);
SVR_mu = mean(SVR)*100;
SVR_std = Confidence_95(SVR)*100;
LOR_mu = mean(LOR)*100;
LOR_std = Confidence_95(LOR)*100;
SWORE_mu = mean(SWORE)*100;
SWORE_std = Confidence_95(SWORE)*100;
% fix the area
x = 0:0.01:1;
w = 0.055;
...
str1=['SVR=', num2str(SVR_mu,3), '$\pm$', num2str(SVR_std,2),'\%'];
str2=['LOR=', num2str(LOR_mu,3), '$\pm$', num2str(LOR_std,2),'\%'];
str3=['SWORE=', num2str(SWORE_mu,3), '$\pm$', num2str(SWORE_std,2),'\%'];
